...
| Map | `ServerScriptService/MapBootstrap.server.lua` | Lobby (shop + upgrade stalls + teleport pad + landmark) and arena (10 cover crates, 6 barrels, 3 dividing walls, a raised catwalk with ramps + full side railings, 10-point spawn ring, a 3x3 grid of overhead lamp posts) — **sealed off from each other, no walkable corridor between them**. Since the teleport pad (see WaveService) is the only way a match actually starts, a corridor physically connecting the two was removed; nothing ever needed to walk through it. `Lighting.Brightness`/`Ambient` raised well above default dusk values specifically because the arena previously had zero light fixtures of its own and read as near-pitch-black even with the global atmosphere tuned — gameplay visibility wins over mood here. Stall/monument name labels now respect wall occlusion and only render within 45 studs (`BillboardGui.AlwaysOnTop = false` + `MaxDistance`) — they previously rendered through walls from anywhere on the map, overlapping the wave/objective UI. Still placeholder blocky geometry — no art pipeline yet. Solid perimeter walls sit flush with the lobby's and arena's actual floor edges (fully sealed now, no doorway gaps), and the arena floor was widened east to actually sit under the barrel cover cluster — closes off every way we'd previously seen someone fall through the map. A `FallSafetyNet` far below everything (Y = -50) is a last-resort backstop on top of that: catches anyone who still ends up falling and teleports them back to the lobby or arena. **A second map/arena variant was intentionally not built this pass** — focus stayed on one complete map + finishing the game-cycle systems around it. |
...
| Client weapon aim pose | `StarterPlayer/StarterPlayerScripts/Controllers/WeaponAimPoseController.lua` | Overrides the local player's own arm `Motor6D` poses so the held weapon always points toward the camera — two-handed grip, no walk-cycle sway in third person, tracks camera pitch up/down in first person — instead of the default walk/idle animation, which doesn't track aim at all. **Read the file's own doc comment before assuming this is complete**: it's local-only (not visible to other players watching you — Motor6D changes from a LocalScript don't replicate; a real fix for that needs an authored Animation asset, which this environment can't produce, or a server-synced pose system), and the exact pose angles are estimates that need real in-Studio visual tuning, not verified numbers. |
